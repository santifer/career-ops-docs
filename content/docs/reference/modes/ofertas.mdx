...
The `ofertas` mode reads multiple JD evaluations (already produced by oferta or auto-pipeline) and ranks them against each other on the six dimensions plus a normalized decision matrix. The output is a side-by-side comparison report and a ranked recommendation.
